...
class WebSocketServer:
    ...
    async def _http_response(self, websocket, request_headers):
        # 检查是否为 WebSocket 升级请求。Connection 可能是 "keep-alive, Upgrade"。
        connection_values = request_headers.headers.get_all("connection")
        connection_tokens = {
            token.strip().lower()
        ...
        if "upgrade" in connection_tokens and upgrade == "websocket":
