...
async def handle_music_command(conn, text):
    ...
    if os.path.exists(MUSIC_CACHE["music_dir"]):
        ...
        if potential_song:
            ...
            # ===== 新增：在线音乐 =====
            try:
                # from plugins_func.functions.online_music import KugouMusic
                # music_path = KugouMusic.download(potential_song)
                #
                # # 把下载的文件加入缓存列表
                # rel_path = os.path.relpath(music_path, MUSIC_CACHE["music_dir"])
                # MUSIC_CACHE["music_files"].append(rel_path)
                #
                # await play_local_music(conn, specific_file=rel_path)
                # return True
                pass
            except Exception as e:
                conn.logger.bind(tag=TAG).error(f"在线音乐失败: {e}")
    # 检查是否是通用播放音乐命令
